...
const hideHealthbarsIfNotVisible = (trackingDescriptor: TrackingDescriptor) => {
  if (!trackingDescriptor.tracking.IsVisible()) {
...
export const renderHealthbars = (): void => {
  ...
  for (const trackingDescriptor of Object.values(state.healthBars)) {
    ...
    if (hideHealthbarsIfNotVisible(trackingDescriptor)) {
      continue;
    } else {
    ...
    if (
      ...
      const percentageHealth =
        trackingDescriptor.tracking.HitPoints /
        trackingDescriptor.tracking.MaxHitPoints;
      ...
      const hue = 130.0 - 130.0 * (1.0 - nonNegativePercentageHealth);
